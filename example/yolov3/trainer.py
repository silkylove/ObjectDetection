...
class Trainer:
    def __init__(self, config):
        ...
        self.img_size = config.model_params['input_size']
        ...
        self.idx2label = dict(enumerate(np.loadtxt(config.label_file, np.object).reshape(-1, ).tolist()))
        self.num_classes = len(self.idx2label)
        self.net = DarkNetYolov3(config.model_params, self.num_classes)
        self.box_coder = Yolov3BboxCoder(self.net)
    ...
    def train(self):
        ...
        losses = AverageMeter()
        self.net.train()
        for i, (imgs, targets, masks) in enumerate(self.loaders['train']):
            imgs = imgs.cuda()
            targets = targets.cuda()
            masks = masks.cuda()

            preds = self.net(imgs)
            loc_loss, cls_loss, loss = self.criterion(preds, targets, masks)
    ...
    def test(self):
        ...
        self.net.eval()
        with torch.no_grad():
            for i, (imgs, targets, masks) in enumerate(self.loaders['test']):
                imgs = imgs.cuda()
                targets = targets.cuda()
                masks = masks.cuda()
